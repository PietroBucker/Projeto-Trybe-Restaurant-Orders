...
class MenuBuilder:
    ...
    def get_main_menu(self, restriction=None) -> List[Dict]:
        ...
        new_list = list()
        # print(lista_dishes[0].recipe)
        # print(self.inventory.check_recipe_availability(lista_dishes[0].recipe))
        for prato in lista_dishes:
            if self.inventory.check_recipe_availability(prato.recipe):
                if (
                    not restriction
                    or restriction not in prato.get_restrictions()
                ):
                    self.inventory.consume_recipe(prato.recipe)
                    new_list.append(
                        {
